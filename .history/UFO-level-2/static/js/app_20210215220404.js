...
function filterClick() {
    ...
    // need to eventually add in all fields to this dictionary
    let filteredValues = {city: cityValue, state: stateValue};

    let filteredData = tableData.filter(values => values.datetime === filteredValues.city);
    filteredData = filteredData.filter(values => values.city === filteredValues.state);
    console.log(filteredData);
    // for (const [key, value] of Object.entries(filteredValues)) {
    //   console.log(`${key}: ${value}`);
    //   if(value !== "") {
        //build table and pass filtered data to each place
        //filter = 'value' from key value pair
}
